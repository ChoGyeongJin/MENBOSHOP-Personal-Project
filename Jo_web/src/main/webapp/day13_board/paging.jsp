...
4. 검색어가 있는 경우외 없는 경우를 구분하여 출력

 -->
 
 
 
 <!-- 검색어가 있는 경우외 없는 경우를 구분하여 출력: searchField, searchWord에 대한 변수선언  -->
 ...
 <c:choose>
    ...
    &nbsp;
    <a href="list.jsp?pageNum=${(pageBlock-2)*pages_pageBlock+1}&pageBlock=${pageBlock-1}&searchField=${searchField}&searchWord=${searchWord}">Previous</a>
    </c:if>
...
<c:if test="${total_pageNum > (pageBlock*pages_pageBlock)}">
    <a href="list.jsp?pageNum=${(pageBlock*pages_pageBlock)+1}&pageBlock=${pageBlock+1}&searchField=${searchField}&searchWord=${searchWord}">NEXT</a>
    &nbsp;
...
<c:if test="${pageNum > 5}">
    ...
    &nbsp;
    <a href="list.jsp?pageNum=${(pageBlock-2)*pages_pageBlock+1}&pageBlock=${pageBlock-1}">Previous</a>
</c:if>
 ...
 <c:forEach var="p" begin="${(pageBlock - 1)*pages_pageBlock +1}" end="${pageBlock*pages_pageBlock}">
    <c:if test="${p le total_pageNum}">
        <a href="list.jsp?pageNum=${p}&pageBlock=${pageBlock}">
            <!-- 현재 페이지를 빨간색으로 출력 -->
            <c:if test="${p eq pageNum}" var="flag"> <!-- flag 변수에는 test 결과값이 저장됨 -->
                <span id="click" style="color:black; font:bold 20px solid">${p}&nbsp;</span>
            </c:if>
            <c:if test="${not flag}">
                ${p}&nbsp;
            </c:if>
...
<!-- 2. 총 페이지번호가 페이지블록을 초과하는 경우 '다음'과 맨 마지막페이지 기호(>>) 출력 -->
<c:if test="${total_pageNum > (pageBlock*pages_pageBlock)}">
    <a href="list.jsp?pageNum=${(pageBlock*pages_pageBlock)+1}&pageBlock=${pageBlock+1}">NEXT</a>
    &nbsp;
